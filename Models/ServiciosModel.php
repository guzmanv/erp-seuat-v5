...
class ServiciosModel extends Mysql{
    ...
    public function selectServicios(){
        ...
        ti.abreviacion_institucion AS Plantel, tp.municipio AS Municipio
        FROM t_servicios ts 
        INNER JOIN t_instituciones AS ti ON ts.id_institucion = ti.id
        INNER JOIN t_planteles AS tp ON ti.id_plantel = tp.id
        INNER JOIN t_categoria_servicios tcs ON ts.id_categoria_servicio = tcs.id  
    ...
    public function insertServicio(string $codigo_servicio, string $nombre_servicio, int $precio_unitario, int $aplica_edo_cuenta, string $anio_fiscal, int $estatus, string $fecha_creacion, string $fecha_actualizacion, int $id_usuario_creacion, int $id_usuario_actualizacion, int $id_institucion, int $id_categoria_servicio, int $id_unidades_medida){
        ...
        $request = $this->select_all($sql);
        if(empty($request)){
            $query_insert = "INSERT INTO t_servicios(codigo_servicio,nombre_servicio,precio_unitario,aplica_edo_cuenta,anio_fiscal,estatus,fecha_creacion,fecha_actualizacion,id_usuario_creacion,id_usuario_actualizacion,id_institucion,id_categoria_servicio,id_unidades_medida) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)";
