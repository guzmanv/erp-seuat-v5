<?php
	class HistorialPagosAlumnoModel extends Mysql
	{
        ...
        public function selectEstudiantes(){
			...
			CONCAT(tgr.numero_natural,' (',sa.nombre_salon,')') AS grado_grupo FROM t_inscripciones AS i
			...
			INNER JOIN t_grados AS tgr ON i.id_grados = tgr.id
		...
		public function selectUltimosMovimientos(int $idAlumno){
			$sql = "SELECT i.id AS id_ingreso,i.observaciones,i.folio,i.fecha_cobro,TIMESTAMPDIFF(SECOND,i.fecha_cobro,NOW()) 
			AS segundos FROM t_ingresos_detalles AS ide
						LEFT JOIN t_ingresos AS i ON ide.id_ingresos = i.id 
						WHERE i.id_persona_paga = $idAlumno AND i.fecha_cobro!= '' ORDER BY i.fecha_cobro DESC
						LIMIT 5";
			$request = $this->select_all($sql);
			return $request;
		}
		...
		public function selectTodosMovimientos(int $idAlumno){
			$sql = "SELECT i.folio, i.fecha_cobro,s.nombre_servicio AS nombre_servicio,spre.nombre_servicio AS nombre_servicio_precarga,id.cargo,per.nombre_persona,per.ap_paterno,per.ap_materno FROM t_ingresos_detalles AS id
			...
			WHERE i.id_persona_paga = $idAlumno ORDER BY i.fecha_cobro DESC";
			$request = $this->select_all($sql);
			return $request;
		}
