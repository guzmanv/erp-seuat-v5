...
function fnGrados(conexion){
    ...
    let url = `${base_url}/Preinscripcion/getGrados?conexion=${conexion}`;
    fetch(url).then((res) => res.json()).then(resultado =>{
...
function fnCampaniaActual(conexion){
    let url = `${base_url}/Preinscripcion/getCampaniaActual?conexion=${conexion}`;
    fetch(url).then((res) => res.json()).then(resultado =>{
...
function fnPlantelSeleccionadoDatatable(){
    ...
    tableInscripciones = $('#tableInscripciones').dataTable( {
        ...
        "ajax":{
            "url": " "+base_url+"/Preinscripcion/getInscripcionesAdmision?plantel="+plantel.value,
            "dataSrc":""
        },
        "columns":[
            {"data":"numeracion"},
...
function fnBtnDesInscribir(idInscripcion,idPersona){
    Swal.fire({
        title: 'Des-inscribir',
        ...
        if (result.isConfirmed) {
            let url = `${base_url}/Preinscripcion/des_inscribir/${idInscripcion}/${idPersona}`;
            fetch(url)
            .then(res => res.json())
            .then((resultado) => {
...
function fnChckColegiaturas(value){
    if(value.checked){
        ...
        let url = `${base_url}/Preinscripcion/getPromocionescolegiaturas/${carreraSeleccionada}`;
        if(carreraSeleccionada != null){
            fetch(url).then((res) => res.json()).then(resultado =>{
                if(resultado.length >0){
                    resultado.forEach(element => {
                        document.querySelector('#div_chck_colegiaturas').innerHTML += "<option value="+0+","+element.id_servicio+","+element.porcentaje_descuento+","+element.cobro_total+">"+element.nombre_promocion+" ( "+element.porcentaje_descuento+" )"+" / "+element.nombre_plantel_fisico+"("+element.abreviacion_institucion+")"+"</option>"
                    });
                }else{
